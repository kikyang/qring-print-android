...
最新版见 [Releases](https://github.com/kikyang/qring-print-android/releases)（v0.7.2，0.90MB，需 Android 13+）。
...
- **图片打印**（Tab 2，v0.6 参数分层）：多选图、单列/双列拼接省纸；抖动三模式（清晰/细腻/高对比）；
  一键增强（Sauvola 自适应二值化，去背景/阴影/手写）；打印浓度（淡/中/浓，就近可调）；
  其余选项（消除笔去批改、自动裁白边、黑白深浅、线稿模式）收进「⚙️ 高级设置」折叠；
  统一画布 **🖌 画布**（涂鸦/文字/图片/条码自由排版，v0.6 单入口，涂鸦并入元素画布）
...
- **其它**（Tab 5，v0.5.1 合并，v0.6 模板系统打通）：
  - **常用模板宫格**（2×2 图标）：课程表 / 单词表 / 每日计划 / 口算题，内置 JSON 注册表（SystemTemplates）数据驱动，一键生成打印
