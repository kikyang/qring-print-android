...
| 候选 | 来源 | 价值 | 评估 |
|---|---|---|---|
| **图片抖动改蛇形扫描** | lztttt v1.6.0 | 高（工作量小） | 交替行方向扩散，消除蠕虫纹/竖条纹；Dither 是纯计算模块，可加 JVM 单测（对齐灰阶直方图 / 中间调块方差） |
...
| # | 标题 | 创建 | 状态 | 备注 |
|---|---|---|---|---|
| **5** | 0.7.4+ 打印对话框前后走纸元素拥挤遮挡，较高图片使打印按钮不可见且不能滚动 | 9-01 | open | **真实 UI bug**（用户附截图）→ 建议 v0.7.7 修：确认条内容区限高 / 对话框可滚动 |
