...
**v0.7.6 已发布**（2026-09-09 推送 main + tag v0.7.6 + GitHub Release）（Android 打印 App + 热敏打印机逆向）。界面主题改为 8 套差异化风格（紫罗兰/墨绿纸感/暗色效能/手账纸感/热敏黑白/奶油多彩/Apple Bento/玻璃拟态；旧微信风/简洁风/蓝白风已移除），我的页主题选择改为竖向列表。**2026-09-09 追加两项**：修复 GitHub issue #5（打印确认对话框遮挡 → 预览图高度自适应 + 内容放进 ScrollView）、图片抖动改蛇形扫描（吸收上游 lztttt v1.6.0），全量 **201 例测试全过**。上游巡检定为**一周一次**（最近 9-09，下次 9-16）。⚠️ **当前最高优先待修 #29**：OTA 主源（jsDelivr data API 版本列表）滞后在 0.7.4，导致 App 检查更新看不到 0.7.6，需改为跨源取最高版本并升版 v0.7.7。
